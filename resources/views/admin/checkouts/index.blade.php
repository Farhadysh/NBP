...
@section('content')
    ...
    <div id="div_search" class="col-md-12 mx-auto">
        <form action="{{route('admin.checkouts.index')}}"
            ...
            <div class="form-group col-md-2">
                <label for="visitor_code">کد بازاریاب</label>
                <input type="text" class="form-control font-small"
                       value="{{request()->has('visitor_code') ? request('visitor_code') : ''}}" id="visitor_code"
                       name="visitor_code"
        ...
        <table class="table shadow-sm">
            ...
            @foreach($checkouts as $checkout)
                <tr class="text-center small {{$checkout->getOriginal('init') == 1 ? 'bg-low-danger' : ''}}">
                    ...
                    <td class="">{{$checkout->position->visitor_code ?? '-'}}</td>
                    <td class="">{{$checkout->user->mobile}}</td>
                    <td class="">
                        @if($checkout->position_id)
                            {{$checkout->position->parent ? $checkout->position->parent->name  : 'NBP'}}
                        @else
                            -
                        @endif
                    </td>
                    <td class="">
                        @if($checkout->position_id)
                            {{$checkout->position->consultant ? $checkout->position->consultant->name : 'NBP'}}
                        @else
                            -
                        @endif
                    </td>
